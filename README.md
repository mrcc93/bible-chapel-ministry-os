...
Phase 2A adds the authentication and authorization foundation without moving sensitive data out of localStorage yet. Existing UI workflows remain usable while `/api/*` requests are guarded by middleware and future `/api/collections/*` routes are now designed to require an authenticated Cloudflare session and role authorization before any D1 CRUD is implemented.

### Recommended auth provider

Use **Cloudflare Access** in front of the Cloudflare Pages app. Access can authenticate users through common identity providers such as Google Workspace, Microsoft Entra ID, Okta, or a one-time PIN policy. This keeps real session handling at Cloudflare's edge and lets Pages Functions read trusted Access headers after the Access policy allows the request.
...
The app now defines three production roles for future D1 access:

| Role | Intended access |
| --- | --- |
| `admin` | Settings, role/admin operations, future migration tooling, and all ministry data. |
...
Sensitive route areas are treated as Admin or Pastor/Leader only:
...
Do not commit secrets or provider credentials to this repository. Configure these in the Cloudflare dashboard or with Wrangler secrets/variables.
