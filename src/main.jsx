...
function App() {
  ...
  const dataStatus = { settings: settingsStatus, rhythm: rhythmStatus, tasks: tasksStatus, events: eventsStatus, annualPlan: annualPlanStatus, services: servicesStatus, people: peopleStatus, series: seriesStatus, bulletin: bulletinStatus, goals: goalsStatus, roadmap: roadmapStatus };
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <p className="sidebar-note">People Directory is D1/API-backed. Visitors, prayer, absences, contacts, attendance, and giving stay local-only until later Phase 2C steps.</p>
    </aside>
    <main className="main">
...
function SettingsPage({ settings, setSettings, dataStatus, apiStatus }) { const planningStatuses = Object.values(dataStatus || {}).filter(status => status?.apiCollection); const errorCount = planningStatuses.filter(status => status.error).length; const planningReady = Boolean(apiStatus?.planningApi?.enabled && apiStatus?.d1?.migrationsApplied); const roleLabel = normalizeRoleLabel(apiStatus?.auth); return <Page eyebrow="Settings" title="Church app setup" description="Rename the app and manage ministry app roles."><Card title="Identity"><div className="form-grid two"><Field label="Church name"><Input value={settings.churchName} onChange={e => setSettings(s => ({ ...s, churchName: e.target.value }))}/></Field><Field label="Pastor / leader"><Input value={settings.pastor} onChange={e => setSettings(s => ({ ...s, pastor: e.target.value }))}/></Field></div><Field label="Current theme"><Input value={settings.theme} onChange={e => setSettings(s => ({ ...s, theme: e.target.value }))}/></Field></Card><MinistryUsersSettings apiStatus={apiStatus}/><Card title="Data status" subtitle="Phase 2C-Prep API readiness"><div className="status-grid"><div><StatusPill tone={planningReady ? 'green' : errorCount ? 'red' : 'gold'}>Planning data</StatusPill><p className="muted">{planningReady ? `D1/API-backed; status confirms planning API is enabled and D1 migrations are applied.` : errorCount && apiStatus?.ok ? 'Authenticated, but one planning request failed.' : errorCount ? 'Planning API is not available. Local fallback is for local/offline development only.' : 'Checking planning API availability.'}</p><p className="muted">Cloudflare Access role: {roleLabel} ({apiStatus?.auth?.roleSource || 'pending'})</p></div><div><StatusPill tone="green">People Directory</StatusPill><p className="muted">People Directory is D1/API-backed for Admin and Pastor/Leader users.</p></div><div><StatusPill tone="gold">Remaining sensitive data</StatusPill><p className="muted">Visitors, prayer requests, absences, pastoral contacts, stats, attendance, and giving remain local-only until later Phase 2C steps.</p></div></div></Card><Card title="Production warning" subtitle="Important before using real pastoral data"><p className="muted">Cloudflare Access controls login. People Directory is migrated to D1; visitors, prayers, absences, contacts, stats, attendance, and giving are not migrated to D1.</p></Card></Page>; }
