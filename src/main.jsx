...
const SUNDAY_WORSHIP_TIME = '10:30 AM';
const SUNDAY_WORSHIP_TIME_LABEL = '10:30 a.m.';
...
const buildServiceFromSermon = (sermon, existing = {}) => {
  ...
  return {
    ...
    time: existing.time || SUNDAY_WORSHIP_TIME,
    sermonId: sermon.id,
...
function addBibleChapelStarterContent({ setRhythm, setTasks, setEvents, setSeries, setBulletin, setPlan, flash }) {
  ...
  setEvents(rows => sortDateAsc(addUnique(addUnique(rows, { title: 'Sunday Worship', date: nextSundayISO(), time: SUNDAY_WORSHIP_TIME, type: 'Service', owner: 'Bible Chapel', notes: 'Weekly worship gathering in Robinson, Illinois.' }), { title: 'Wednesday Bible Study', date: todayISO(), time: '6:00 PM', type: 'Meeting', owner: 'Bible Chapel', notes: 'Midweek Bible study and ministry night.' })));
...
function Dashboard({ settings, stats, events, services, visitors, prayers, contacts, absences, tasks, goals, plan, series, bulletin, setView, apiStatus, setTasks, setRhythm, setEvents, setSeries, setBulletin, setPlan, flash }) {
  ...
  return <Page eyebrow="Ministry Command Center" title={greeting}>
    ...
    <div className="stat-grid">
      <Metric label="Next Sunday" value={nextService ? 'Planned' : SUNDAY_WORSHIP_TIME_LABEL} meta={nextService ? `${fmtDate(nextService.date)} · ${nextService.title}` : 'Create this Sunday’s service plan'}/>
...
function Sunday({ services, setServices, series, settings, flash, dataStatus }) {
  ...
  const createService = () => { const svc = { id: uid(), date: nextSundayISO(), time: SUNDAY_WORSHIP_TIME, title: 'Sunday Worship', order: cloneServiceOrder(), songs: [], slides: [] }; setServices(rows => [svc, ...rows]); setActiveId(svc.id); };
